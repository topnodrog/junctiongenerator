...
The "certification" / verification model is the Rust crate **`jgc_verifier`** at `packages\jgc-node\rust\` — a ZK-SNARK (Groth16/BN254) verifier compiled to WASM. It is location-independent (no hardcoded paths; deps from crates.io). Moving the repo would NOT break it; worst case is `cargo build` to regenerate `target/`. Staying on Windows = zero risk.
