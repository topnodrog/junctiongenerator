# Cold Outreach Email Templates for JGC

> **Accuracy guardrail (2026-08-15):** An early, valueless zero-premine JGTC
> public pilot is live across two independent seeds and a workstation runner.
> Current receipts record participation but are not production-sound useful-
> compute proofs. Bonded rewards, slashing, and JGC mainnet are not active.
> Update every template with measured facts before sending.

## Template 1: Crypto-Native VCs (Paradigm, a16z, Polychain)

**Subject Line Options:**
- "Layer-1 junctioning + collusion-resistant verification"
- "Crypto verification failures remain costly — we shipped an early public pilot"
- "Deterministic replay + signed participation evidence (JGTC pilot)"

---

**Email Body:**

```
Hi [Name],

Crypto scams, exploits, and oracle failures remain costly. Many systems still
depend on opaque execution or a small number of trusted verifiers.

We're solving this with Proof of Useful Computation — a Layer-1 where verification 
work (AI inference) becomes consensus. Deterministic execution + multi-challenger 
quorum produces post-quantum signed, consensus-committed evidence.

We shipped a local testnet in June and launched an early public JGTC pilot in
August across independent seeds. Its current signed receipts record testnet
participation; production-sound useful-compute proofs and bonded validator
economics remain milestones.

Your portfolio (Eigen Layer, Lido) suggests you care about consensus security and 
economic incentives. This is that problem from a different angle.

Would be great to grab 15-20 minutes to show you what we built. Are you around 
Thursday or Friday this week?

—James
junctiongenerator.net
james_gordon@junctiongenerator.net
```

**Variations by Firm:**

**For a16z Crypto:**
```
Your thesis on "the state of crypto infrastructure" aligns with what we're solving. 
JGC is verification infrastructure that scales Layer-1s through useful computation.
```

**For Paradigm:**
```
You backed Eigen Layer (validator security). JGC is the flip side — 
making validators honest through collusion-resistant quorum. Worth a look.
```

**For Polychain:**
```
Your focus on "novel consensus + economic security" is exactly what JGC is building. 
Public JGTC pilot live, junctioning shipped, ready to show you.
```

---

## Template 2: Traditional VCs (Sequoia, Khosla, etc.)

**Subject Line Options:**
- "Addressing crypto's verification gap with replayable evidence"
- "AI inference as a consensus mechanism"
- "We built a Layer-1 verification network (live demo)"

---

**Email Body:**

```
Hi [Name],

We're building JunctionGenerator — a verification infrastructure for crypto that 
solves two problems:

1. **Fraud Risk Reduction** — costly losses where execution or oracle evidence is weak
2. **Enterprise Adoption** — Smart contracts need independently auditable
   execution evidence for institutional settlement

Our approach: Make useful work (AI inference) the basis for consensus. Validators 
replay sampled work and commit signed quorum evidence. The current pilot
records test participation; production-sound proofs and bonded penalties are the next
consensus milestone.

We're pre-revenue but shipped product:
- Local/private testnet validated (June 2026)
- Early public JGTC pilot launched (August 2026; soak in progress)
- Junctioning Layer-1 working with local inference
- Consensus-v2 evidence path (replay + delayed sampling + signed quorum)

We're raising seed funding for pilot hardening, external review, and enterprise research pilots.

Think this is worth 15-20 minutes to discuss?

—James Gordon
Founder, JunctionGenerator
james_gordon@junctiongenerator.net
junctiongenerator.net
```

**Why this works for traditional VCs:**
- Leads with a concrete problem; use a current cited figure only after verifying it
- No crypto jargon (or explains it)
- Shipped proof (credibility)
- Clear use case (enterprise settlement)
- Reasonable ask ($1M, not $100M)

---

## Template 3: Crypto Angels / Protocol Engineers

**Subject Line Options:**
- "Deterministic verification + consensus-committed audit evidence"
- "Built junctioning Layer-1 + quorum consensus (want to see it?)"
- "Signed quorum verification (local testnet)"

---

**Email Body:**

```
Hi [Name],

Quick one — building JunctionGenerator, a verification infrastructure that makes 
useful work the basis for consensus.

The core insight: Validators replay sampled AI work, sign observations, and
commit quorum evidence. Bonded rewards and penalties are the next consensus
milestone. No proof-of-work waste and no single-validator oracle.

You've built [relevant protocol]. You probably see this problem — verification is 
the hard part.

We shipped the local evidence path: junctioning, delayed sampling, signed
quorum, block commitment, sync, and adversarial rejection.

Raising $1M seed. Would love your thoughts on the approach — 15 min if you're game?

—James
james_gordon@junctiongenerator.net
```

**Personalization for Different Angels:**

**For a Protocol Engineer:**
```
You architected [protocol]. You probably think about consensus differently now. 
JGC's multi-challenger quorum solves the "one validator can't be trusted" problem 
without proof-of-work. Worth a look.
```

**For a Founder:**
```
You raised for [project]. You know the validator security problem intimately. 
JGC is approaching it through useful compute and consensus-committed audit evidence. Local validation is promising.
```

**For a Crypto Researcher:**
```
You wrote about [consensus/verification topic]. Our approach bridges your thesis 
with economic incentives. Would love your technical feedback.
```

---

## Subject Line Strategies

**What Gets Opened:**
- Specific problem + proof ("Costly verification gap — local audit layer shipped")
- Relevant portfolio reference ("You backed Eigen, here's the verification flip-side")
- Technical intrigue ("Deterministic replay + collusion-resistant quorum")

**What Gets Deleted:**
- "Disruptive AI/Web3/Blockchain" (generic)
- "Exciting opportunity" (salesy)
- "Quick favor?" (spammy)
- "We should talk" (why would they?)

**Best performers (for crypto):**
- Lead with a metric only when it is current, scoped, and cited
- Lead with shipped proof ("Local testnet validated; audit consensus shipped")
- Lead with relevant problem ("Validator collusion prevention")

---

## Follow-Up Sequence (If No Response)

**Day 3:**
```
Hi [Name],

Quick follow-up on the JGC email. No pressure — just wanted to flag it in case 
it got buried.

Appreciate your time if you get a chance to look.

—James
```

**Day 7:**
```
Last note — shipping updates on junctioning if you're interested in the space.

Best,
James
```

Then move on. If they don't reply by Day 7, they're not interested.

---

## When You DO Get a Response

### If They Say "Tell me more:"
```
Hi [Name],

Great — appreciate the interest. Three quick points:

1. **Problem:** Crypto scams and exploits remain costly. Execution evidence can
   be opaque, oracles can be centralized, and validators can collude.

2. **What we built:** An early public JGTC pilot across independent seeds. It
   records signed participation while production-sound useful-compute proofs
   and bonded enforcement remain future work.

3. **Why it matters:** Targets better execution evidence for DeFi, enterprise
   smart-contract auditability, and AI-model tamper detection.

Would love to walk you through it live — 15-20 min call Friday?

—James
```

### If They Ask "What's the ask?"
```
$1M seed round to accelerate:
- Bonded validator consensus + public-pilot hardening (date not committed)
- Enterprise pilot partnerships
- Exchange integration

Use of funds: Product dev ($400K), validator incentives ($400K), go-to-market 
($150K), ops ($50K).

Interested in exploring?

—James
```

### If They Ask "What's the token model?"
```
JGC coin:
- Fixed supply (non-inflationary after genesis)
- 16-decimal accounting
- Bonded validator rewards and penalties (planned; not active)
- A possible burn/penalty policy is still under design

Not launching with token on Day 1 — focusing on network security first.

Worth a call to discuss economics in detail?

—James
```

---

## Pro Tips

**Send from your own email** (not a generic contact form)
- Personalizes the email
- Ensures replies come back to you
- Looks more legitimate

**Send one-to-one, not mass CC**
- Use BCC if sending batch
- Each email feels personal

**Best time to send:** 
- Tuesday-Thursday, 9am-11am their timezone
- Avoid Monday (busy) and Friday (not reading)

**Optimal frequency:**
- Max 3 emails per day (sending too many looks spammy)
- Space them out: morning batch, afternoon batch

**Include a clear next step:**
- "Are you around Thursday for a quick call?"
- Specific dates/times if possible
- Use Calendly link if you want (but optional)

**Don't:** Attach PDFs or decks
- Use links: "Here's the deck: [junctiongenerator.net/pitch]"
- Attachments = immediately trashed

---

## Customization Checklist

Before sending each email, verify:

- [ ] Recipient's name spelled correctly
- [ ] Company/firm is mentioned (shows you researched)
- [ ] One personal detail (their investment, their protocol) included
- [ ] Subject line is specific (not generic)
- [ ] Call-to-action is clear (what's the next step?)
- [ ] Email is <150 words
- [ ] No attachments
- [ ] No "Dear Sir/Madam" or other generic openings
- [ ] Tone is informational, not salesy
- [ ] You've proofread (typos kill credibility)

---

## Success Formula

**Subject:** [Problem or Proof]  
**Body:** [Hook] + [What you shipped] + [Why they should care] + [Clear ask]  
**Tone:** Informational, not desperate  
**Length:** 100-150 words  
**CTA:** Specific time/day for 15-min call

This works because:
1. You're not asking for money upfront
2. You're showing what has shipped (credibility)
3. You're aligning with their thesis (relevance)
4. You're not wasting their time (clear ask)

**That's it.** Go forth and don't apologize for what you've built.
