 ...
 * prevention, coinbase maturity, and ML-DSA (post-quantum) spend authorization.
 */
...
function fund(): { utxo: UTXOSet; cb: string } {
  const coinbase: Transaction = {
    version: 1, inputs: [],
    outputs: [{ value: J(10n), scriptPubKey: pqScriptPubKey(alice.publicKey) }],
    locktime: 0,
  };
...
function sign(tx: Transaction, key: { privateKey: string; publicKey: string }): Transaction {
  tx.inputs[0]!.scriptSig = pqScriptSig(pqSignHash(key.privateKey, txSigHash(tx)), key.publicKey);
  return tx;
}
...
describe("UTXO ledger", () => {
  test("valid signed spend conserves value and reports the fee", () => {
    ...
    const tx = sign(spend(cb, [
      ...
      { value: J(4n) - 1000n, scriptPubKey: pqScriptPubKey(alice.publicKey) },
    ]), alice);
  ...
  test("applyTransaction spends the input and creates outputs", () => {
    const { utxo, cb } = fund();
    const tx = sign(spend(cb, [{ value: J(6n), scriptPubKey: pqScriptPubKey(bob.publicKey) }]), alice);
  ...
  test("overspend (inflation) is rejected", () => {
    const { utxo, cb } = fund();
    const tx = sign(spend(cb, [{ value: J(11n), scriptPubKey: pqScriptPubKey(bob.publicKey) }]), alice);
    const r = validateSpend(tx, utxo, MATURE);
  ...
  test("spending an unknown output is rejected", () => {
    const { utxo } = fund();
    const tx = sign(spend("ff".repeat(32), [{ value: J(1n), scriptPubKey: pqScriptPubKey(bob.publicKey) }]), alice);
    expect(validateSpend(tx, utxo, MATURE).ok).toBe(false);
  });
  ...
  test("spend signed by the wrong key is rejected (no theft)", () => {
    const { utxo, cb } = fund();
    const tx = sign(spend(cb, [{ value: J(6n), scriptPubKey: pqScriptPubKey(bob.publicKey) }]), bob); // Bob signs Alice's coin
    expect(validateSpend(tx, utxo, MATURE).ok).toBe(false);
  });
  ...
  test("duplicate input within a tx is rejected", () => {
    ...
    const tx: Transaction = {
      ...
      outputs: [{ value: J(6n), scriptPubKey: pqScriptPubKey(bob.publicKey) }],
      locktime: 0,
    };
    expect(validateSpend(tx, utxo, MATURE).ok).toBe(false);
  });
  ...
  test("sighash ignores scriptSig contents (signature can't sign itself)", () => {
    const { cb } = fund();
    const base = spend(cb, [{ value: J(6n), scriptPubKey: pqScriptPubKey(bob.publicKey) }]);
